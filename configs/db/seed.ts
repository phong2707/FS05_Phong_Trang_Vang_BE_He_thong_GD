...
async function seed() {
  try {
    ...
    const studentNames = Array.from({ length: 40 }, (_, i) => `Demo HọcVien${i + 1}`);
    ...
    const courseWeb = await models.course.create({
      data: {
        title: "Kỹ sư Lập trình Web Fullstack (MERN Stack)", description: "Khóa học toàn diện từ con số 0 trở thành kỹ sư Fullstack.",
        price: 8500000, discountPrice: 6500000, startDate: new Date("2026-06-01"), durationValue: 6, durationUnit: "MONTH", daysOfWeek: "T3, T5, T7",
        level: "BEGINNER", maxStudents: 50, isFeatured: true, isSequential: true, status: "PUBLISHED", adminId: admin.id, categoryId: subCatWeb.id, thumbnailUrl: "https://placehold.co/800x400/2563eb/white?text=Fullstack+MERN",
      }
    });

    const courseQA = await models.course.create({
      data: { title: "Software Testing chuyên sâu", description: "Làm chủ Selenium, Cypress và Postman.", price: 5500000, status: "PUBLISHED", adminId: admin.id, categoryId: subCatQA.id, thumbnailUrl: "https://placehold.co/800x400/16a34a/white?text=Automation+Test" }
    });

    const courseUIUX = await models.course.create({
      data: { title: "Figma UI/UX Design Masterclass", description: "Thiết kế giao diện người dùng đỉnh cao.", price: 3200000, status: "PUBLISHED", adminId: admin.id, categoryId: subCatUIUX.id, thumbnailUrl: "https://placehold.co/800x400/db2777/white?text=UI+UX+Design" }
    });
    ...
    // ==========================================
    const subFrontend = await models.subject.create({ data: { courseId: courseWeb.id, name: "Frontend với ReactJS", sortOrder: 1, isSequential: true, allowReview: true, teachers: { create: [ { teacherId: teachers[0].id, type: "MAIN" } ] } } });
    const subBackend = await models.subject.create({ data: { courseId: courseWeb.id, name: "Backend Core", sortOrder: 2, isSequential: true, allowReview: false, teachers: { create: [ { teacherId: teachers[1].id, type: "MAIN" } ] } } });
    const subDatabase = await models.subject.create({ data: { courseId: courseWeb.id, name: "Database Design", sortOrder: 3, isSequential: false, allowReview: true, teachers: { create: [ { teacherId: teachers[1].id, type: "MAIN" } ] } } });
    const subAutomation = await models.subject.create({ data: { courseId: courseQA.id, name: "API Automation", sortOrder: 1, isSequential: false, allowReview: true, teachers: { create: [ { teacherId: teachers[2].id, type: "MAIN" } ] } } });
    const subUixFoundation = await models.subject.create({ data: { courseId: courseUIUX.id, name: "UI Foundation", sortOrder: 1, isSequential: false, allowReview: true, teachers: { create: [ { teacherId: teachers[2].id, type: "MAIN" } ] } } });
    ...
    const chapDB1 = await models.chapter.create({ data: { subjectId: subDatabase.id, title: "Chương 1: Chuẩn hóa dữ liệu", sortOrder: 1 } });
    const chapQA1 = await models.chapter.create({ data: { subjectId: subAutomation.id, title: "Chương 1: Postman", sortOrder: 1 } });
    const chapUI1 = await models.chapter.create({ data: { subjectId: subUixFoundation.id, title: "Chương 1: Layout", sortOrder: 1 } });
    ...
    const v2 = await models.video.create({ data: { chapterId: chapFE1.id, title: "Bài 2: Render & JSX", videoUrl: "https://www.youtube.com/watch?v=SqcY0GlETPk", durationSeconds: 2400, provider: "YOUTUBE", sortOrder: 2 } });
    
    const vBE1 = await models.video.create({ data: { chapterId: chapBE1.id, title: "Bài 1: API", videoUrl: "https://www.youtube.com/watch?v=l8WPWK9mS5M", durationSeconds: 1800, provider: "YOUTUBE", sortOrder: 1 } });
    const vDB1 = await models.video.create({ data: { chapterId: chapDB1.id, title: "Bài 1: CSDL", videoUrl: "https://www.youtube.com/watch?v=UrYLYV7WSHM", durationSeconds: 1600, provider: "YOUTUBE", sortOrder: 1 } });
    const vQA1 = await models.video.create({ data: { chapterId: chapQA1.id, title: "Bài 1: Test", videoUrl: "https://www.youtube.com/watch?v=VywxIQ2ZXw4", durationSeconds: 1400, provider: "YOUTUBE", sortOrder: 1 } });
    const vUI1 = await models.video.create({ data: { chapterId: chapUI1.id, title: "Bài 1: Design", videoUrl: "https://www.youtube.com/watch?v=c9Wg6Cb_YlU", durationSeconds: 1500, provider: "YOUTUBE", sortOrder: 1 } });
    ...
    const q1 = await models.question.create({
      data: {
        scope: "CHAPTER", chapterId: chapFE1.id, teacherId: teachers[0].id, typeId: typeSingle.id, content: "Hook nào dùng để call API?",
        explanation: "useEffect", difficulty: "EASY",
        answers: { create: [ { answerText: "useState", isCorrect: false, orderIndex: 1 }, { answerText: "useEffect", isCorrect: true, orderIndex: 2 } ] }
      }
    });

    const q2 = await models.question.create({
      data: { scope: "SUBJECT", subjectId: subFrontend.id, teacherId: teachers[0].id, typeId: typeEssay.id, questionFormat: "ESSAY", content: "Hãy upload file", difficulty: "HARD" }
    });

    const testChap1 = await models.test.create({ data: { scope: "CHAPTER", chapterId: chapFE1.id, title: "Quiz 1", testType: "QUIZ", durationMinutes: 15, testQuestions: { create: [{ questionId: q1.id, points: 10, sortOrder: 1 }] } } });
    const testSubject = await models.test.create({ data: { scope: "SUBJECT", subjectId: subFrontend.id, title: "Bài thi Đồ án", testType: "ESSAY", durationMinutes: 120, testQuestions: { create: [{ questionId: q2.id, points: 10, sortOrder: 1 }] } } });

    // ==========================================
    // 9. LỚP HỌC & DATA HÀNG LOẠT (✅ FULL 40 STUDENTS)
    // ==========================================
    const classFS = await models.classGroup.create({ data: { subjectId: subFrontend.id, name: "Lớp Fullstack K45 - Tối 3-5-7", startDate: new Date("2026-06-05"), endDate: new Date("2026-12-05"), maxStudents: 35, roomLink: "https://zoom.us/j/123456" } });
    const classBE = await models.classGroup.create({ data: { subjectId: subBackend.id, name: "Lớp Backend K45 - Tối 2-4-6", startDate: new Date("2026-06-07"), endDate: new Date("2026-12-07"), maxStudents: 30, roomLink: "https://meet.google.com/backend" } });
    const classDB = await models.classGroup.create({ data: { subjectId: subDatabase.id, name: "Lớp Database K45 - Cuối tuần", startDate: new Date("2026-06-08"), endDate: new Date("2026-12-08"), maxStudents: 28, roomLink: "https://zoom.us/j/987" } });
    const classQA = await models.classGroup.create({ data: { subjectId: subAutomation.id, name: "Lớp QA Automation K46", startDate: new Date("2026-07-01"), endDate: new Date("2026-11-30"), maxStudents: 32, roomLink: "https://meet.google.com/qa" } });
    const classUIUX = await models.classGroup.create({ data: { subjectId: subUixFoundation.id, name: "Lớp UIUX K20", startDate: new Date("2026-07-03"), endDate: new Date("2026-10-30"), maxStudents: 25, roomLink: "https://zoom.us/j/uiux" } });
    ...
    console.log("🔗 Đang phân bổ 40 sinh viên vào Lớp, tạo Điểm và Lịch sử Giao dịch...");
    ...
    for (let i = 1; i < students.length; i++) {
      ...
      const enroll = await models.courseEnrollment.create({ data: { userId: student.id, courseId: target.crs.id, status: "ACTIVE", progress: Math.floor(Math.random() * 100) } });
      await models.classGroupUser.create({ data: { userId: student.id, classGroupId: target.cls.id, role: "STUDENT" } });

      // Giao dịch (Doanh thu)
      await models.transaction.create({
        data: {
          studentId: student.id, courseId: target.crs.id, enrollmentId: enroll.id, amount: target.crs.price,
          paymentMethod: i % 2 === 0 ? "VNPAY" : "MANUAL", status: "SUCCESS", referenceCode: `VNPAY_DEMO_${i}`,
          createdAt: new Date(Date.now() - Math.floor(Math.random() * 30) * 24 * 60 * 60 * 1000) // Random trong 30 ngày qua
        }
      });
      ...
      await models.subjectGrade.create({
        data: {
          subjectId: target.subj.id, classGroupId: target.cls.id, studentId: student.id, status: "PUBLISHED", updatedById: teachers[0].id, publishedAt: new Date(),
    ...
    const sched1 = await models.schedule.create({ data: { classGroupId: classFS.id, teacherId: teachers[0].id, title: "Buổi 1: Tổng quan ngành Web", startAt: new Date(), endAt: new Date(Date.now() + 7200000), dayOfWeek: 3 } });
    ...
    await models.submission.create({ data: { testId: testChap1.id, studentId: specificStudent.id, classGroupId: classFS.id, score: 10, status: "GRADED", finalScoreStatus: "AUTO_GRADED", userAnswers: { create: [ { questionId: q1.id, answerId: ansQ1?.id, isCorrect: true } ]} } });
